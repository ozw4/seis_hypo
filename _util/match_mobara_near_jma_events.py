		...
		if (
			...
			if best_event_id in das_by_event_id:
				if not np.isclose(das_by_event_id[best_event_id], das_score):
					raise ValueError(
						f'event_id={best_event_id} に複数の異なる das_score が対応しています'
					)
			else:
...
def attach_das_score(
	df_ep_subset: pd.DataFrame, das_by_event_id: dict[int, float]
) -> pd.DataFrame:
...
def main(
	...
	epic_path: str | Path,
	meas_path: str | Path,
	out_path_epic: str | Path,
	out_path_meas: str | Path,
) -> None:
	"""Excel と JMA arrivetime_epicenters の対応から
	対応が取れた event_id のみを含む epicenters 抽出版と
	対応する measurements 抽出版を出力する。
	"""
	excel_path = Path(excel_path)
	epic_path = Path(epic_path)
	meas_path = Path(meas_path)
	out_path_epic = Path(out_path_epic)
	out_path_meas = Path(out_path_meas)

	if not excel_path.is_file():
		raise FileNotFoundError(f'Excel ファイルが見つかりません: {excel_path}')
	if not epic_path.is_file():
		raise FileNotFoundError(f'epicenters CSV が見つかりません: {epic_path}')
	if not meas_path.is_file():
		raise FileNotFoundError(f'measurements CSV が見つかりません: {meas_path}')

	df_xl = pd.read_excel(excel_path)
	df_ep = pd.read_csv(epic_path)
	df_meas = pd.read_csv(meas_path)

	matched_event_ids, das_by_event_id = match_excel_to_epicenters_with_das(
		df_xl,
		df_ep,
		time_tol_sec=1.0,
		lat_tol_deg=0.1,
	...
	# epicenters 側
	df_ep_subset = subset_by_event_ids(df_ep, matched_event_ids)
	if 'origin_time_dt' in df_ep_subset.columns:
		df_ep_subset = df_ep_subset.drop(columns=['origin_time_dt'])
	df_ep_subset = attach_das_score(df_ep_subset, das_by_event_id)

	out_path_epic.parent.mkdir(parents=True, exist_ok=True)
	df_ep_subset.to_csv(out_path_epic, index=False)
	print(f'[output] wrote subset epicenters CSV to: {out_path_epic}')

	# measurements 側
	df_meas_subset = subset_by_event_ids(df_meas, matched_event_ids)
	out_path_meas.parent.mkdir(parents=True, exist_ok=True)
	df_meas_subset.to_csv(out_path_meas, index=False)
	print(f'[output] wrote subset measurements CSV to: {out_path_meas}')


if __name__ == '__main__':
	excel_path = Path('/home/dcuser/mobara2025/data/EQ_widerange_with_tauP_dist.xlsx')
	epic_path = Path('/workspace/data/arrivetime/arrivetime_epicenters.csv')
	meas_path = Path('/workspace/data/arrivetime/arrivetime_measurements.csv')
	out_path_epic = Path('arrivetime_epicenter_mobara2020.csv')
	out_path_meas = Path('arrivetime_measurements_mobara2020.csv')
	main(excel_path, epic_path, meas_path, out_path_epic, out_path_meas)
