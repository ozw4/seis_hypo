...
def _plot_histograms(
	...
	for col in metrics:
		...
		# 平均・中央値の縦線を追加
		plt.axvline(
			mean_val, linestyle='--', linewidth=1.5, label=f'mean={mean_val:.2f}'
		)
		plt.axvline(
			median_val,
			linestyle=':',
...
def plot_event_quality(
	...
	*,
	...
	"""Df 内の任意のカラム名を指定して、震源品質と座標誤差のプロット一式を作る wrapper。

	作る図:
	  - ヒストグラム: RMS / ERH / ERZ（hist_ranges で範囲指定可能）
	  - vs NWR: RMS / ERH / ERZ vs NWR
	  - 幾何: RMS vs DMIN, RMS vs GAP
	  - vs depth: RMS / ERH / ERZ vs depth_col
	  - 空間分布: XY 上で RMS を色付け
	  - 座標誤差: JMA vs HYPOINV の水平距離・深さ差・RMS との関係など
	"""
	out = Path(out_dir)
