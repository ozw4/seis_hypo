...
def build_epic_from_eqt_picks(
	...
	if mode == 'network_center':
		...
		if eqt_p.empty:
			...
			lat.loc[first_p.index] = first_p['lat'].values
			lon.loc[first_p.index] = first_p['lon'].values
	...
	df_epic_eqt = pd.DataFrame(
		dict(
			...
			origin_time=origin_time.values,
			latitude_deg=lat.values,
			longitude_deg=lon.values,
			depth_km=depth.values,
		)
	)
