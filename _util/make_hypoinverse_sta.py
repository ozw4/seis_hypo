...
def parse_station_code(code: str):
	"""station_code から Hypoinverse 用の site(5文字) と net(1文字) を取り出す。

	ルール:
	  - s = station_code.strip().upper()
	  - sta = s[:5]
	  - len(s) == 6 のとき net = s[5]
	  - len(s) < 6 のとき net = ''
	  - 7文字以上は存在しない前提
...
def decimal_deg_to_lat_fields(lat):
...
def decimal_deg_to_lon_fields(lon):
...
def write_hypoinverse_sta(
	...
	for _, row in df.iterrows():
		...
		# 可変パラメータ（補正量を含む）を station.csv から取得
		chan = (
...
if __name__ == '__main__':
	write_hypoinverse_sta(
		...
		output_sta='stations_hypoinverse.sta',
		default_channel='HHZ',
		default_comp1='Z',
		default_period=1.0,
	)
