	...
	*,
	...
	for sta, d in by_sta.items():
		...
		if missing and fill_missing_with_zeros:
			n_fill += 1
			print(
				f'[WARN] PhaseNet input: station={sta} missing={missing} -> fill zeros'
			)
	...
	if not out:
		raise ValueError('no station usable for PhaseNet (need at least 1 of U/Z/N/E)')

	if n_fill > 0:
		print(f'[INFO] PhaseNet input: filled missing components for {n_fill} stations')
