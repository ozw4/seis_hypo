...
def minute0_from_event_dir_name(dir_name: str) -> str | None:
...
def event_step1_ok(event_dir: Path) -> bool:
...
def _scan_orphan_dirs(
	*, win_event_dir: Path, dmin: date | None, dmax: date | None
	...
	for d in list_event_dirs(
		...
		minute0_from_dir = minute0_from_event_dir_name(d.name) or ''
...
def _request_downloads_with_retry(
	*,
	minute_list: list[str],
	cfg: JmaStep1RescueDownloadConfig,
	target_origin_ns_set: set[int],
	orphan_dirname_set: set[str],
) -> list[dict[str, object]]:
	...
	for i, m0 in enumerate(minute_list, 1):
		...
		print(
			f'\n[req {i}/{len(minute_list)}] get_event_waveform {m0}..{m1}',
			flush=True,
		)
		...
		for k in range(1, int(cfg.max_retry_get_event_waveform) + 1):
			...
			try:
				...
				last_err = repr(e)
				print(
					'[warn] get_event_waveform failed -> '
					f'retry {k}/{cfg.max_retry_get_event_waveform}: {last_err}',
					flush=True,
				)
			...
			if useful <= 0:
				...
				print(
					'[warn] no useful dirs -> '
					f'retry {k}/{cfg.max_retry_get_event_waveform}',
					flush=True,
				)
				time.sleep(float(cfg.retry_sleep_sec) * float(k))
				continue
...
def run_step1_rescue_download(cfg: JmaStep1RescueDownloadConfig) -> None:
	...
	write_csv(cfg.out_orphan_dirs_csv, orphan_rows)
	print(
	...
	for _, row in epi.iterrows():
		...
		if cfg.skip_if_already_ok and event_step1_ok(event_dir):
			ok += 1
			continue
	...
	write_csv(cfg.out_rescue_targets_csv, target_rows)
	print(
	...
	run_rows = _request_downloads_with_retry(
	...
	write_csv(cfg.out_rescue_run_csv, run_rows)
	print(
